...
# Actualización del total de billetes tras extraer un billete de $1000
total_billetes_post_M1 <- total_billetes - 1
# Cálculo de la probabilidad de extraer un billete de $500 después de haber extraído uno de $1000
P_Q2_given_M1 <- billetes_500 / total_billetes_post_M1
# Imprimir la probabilidad redondeada a cuatro decimales
print(sprintf("La probabilidad de que el segundo billete sea de $500 dado que el primero fue de $1000 es: %.4f", P_Q2_given_M1))

P_C1 <- 56 / 183
P_D2_given_C1 <- 36 / 182
P_C1_and_D2 <- P_C1 * P_D2_given_C1
P_C1_and_D2
```
...
costo_reciclaje <- 4.5
# Costo esperado para 50,000 piezas
costo_esperado <- 50000 * p_defecto_total * costo_reciclaje
# b) Probabilidad de que un artículo defectuoso haya sido soldado por el robot C (P(C|Defecto))
p_C_dado_defecto <- (p_defecto_C * prop_C) / p_defecto_total

print(sprintf("a) El costo esperado por artículos defectuosos en un lote de 50,000 piezas es: %.2f pesos", costo_esperado))
print(sprintf("b) La probabilidad de que un artículo defectuoso haya sido soldado por el robot C es: %.4f", p_C_dado_defecto))
```
...
mu <- 18.0
sigma <- 1.25
# Probabilidad de que X sea menor o igual a 21.75
p_less_21_75 <- pnorm(21.75, mean = mu, sd = sigma)
# Probabilidad de que X esté entre 15 y 20
p_between_15_20 <- pnorm(20, mean = mu, sd = sigma) - pnorm(15, mean = mu, sd = sigma)
# Probabilidad de que X sea mayor a 17

p_greater_17 <- 1 - pnorm(17, mean = mu, sd = sigma) # Imprimir resultados
p_less_21_75
p_between_15_20
p_greater_17
```
...
El volumen del vaso es de 0.32 litros y queremos que el desbordamiento ocurra en menos del 3% de los casos.
```{r}
# Parámetros conocidos
volumen_vaso <- 0.32
desviacion_estandar <- 0.015
probabilidad_desborde <- 0.03
# Calcular el cuantil que corresponde al 97% (1 - 0.03)
z <- qnorm(0.97)
...
## Introducción:
Este documento calcula la probabilidad de que al menos cuatro de seis amas de casa seleccionadas al azar reciban una revista doméstica, dado que la probabilidad de que una ama de casa seleccionada reciba la revista es del 75%.

## Cálculo de Probabilidades:
Utilizamos la función pbinom para calcular la probabilidad acumulativa de una distribuciónbinomialconparámetrossize = 6yprob = 0.75.
```{r}
# Definir los parámetros de la distribución binomial
n <- 6
p <- 0.75
# Calcular la probabilidad de que al menos cuatro amas de casa reciban la revista
probabilidad <- 1 - pbinom(3, size = n, prob = p) # pbinom da P(X <= k), por lo que 1 - P(X <= 3) da P(X >= 4)
probabilidad
```
